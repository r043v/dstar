...
void gp_clearRingbuffer() {
    int i;
    u32 *SEGMENT0 = (u32*)SAMPLEBUFFER0;
...
volatile int lastSEG = 0;

void gp_addRingSegment( u16 *add_buffer ) {
	 u32 SEGMENT1 = SAMPLEBUFFER0 + SEGMENTSIZE;
     if (lastSEG==0) {
     	while (rDCSRC2>=SEGMENT1);
     	asm_memcpy((void*)SEGMENT1,add_buffer,SEGMENTSIZE);
     	lastSEG = 1;
     }
     else {
     	while (rDCSRC2<SEGMENT1);
     	asm_memcpy((void*)SAMPLEBUFFER0,add_buffer,SEGMENTSIZE);
     	lastSEG = 0;
     }
}

u16* gp_getRingSegment() {
	 u32 SEGMENT1 = SAMPLEBUFFER0 + SEGMENTSIZE;
     if (lastSEG==0) {
     	while (rDCSRC2>=SEGMENT1);
     	lastSEG = 1;
     	...
     	while (rDCSRC2<(u32)SEGMENT1);
     	lastSEG = 0;
     	return (u16*)SAMPLEBUFFER0;
     }
}
