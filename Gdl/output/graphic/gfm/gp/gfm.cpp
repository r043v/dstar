...
{	y = 239-y; u32 y1 = y-sy; u32 x1 = x+sx;
	...
	{	limit->scrStart[c]	= &( (screenBuffer[c])[x*240 + y] );
		limit->scrEnd[c]	= &( (screenBuffer[c])[x1*240 + y1 - 1]); 
	};
...
{	if(x>319||x1>319||y>239||y1>239) Gdl_showMsg("setBlitLimit error! %i %i %i %i",x,y,x1,y1);
	...
	for(u32 c=0;c<2;c++)
	{	scrStart[c]	= &( (screenBuffer[c])[x*240 + y]   );
		scrEnd[c]	= &( (screenBuffer[c])[x1*240 + y1 - 1] );
	};
...
{    
     ...
     if(x < (frmWidthStart-(int)sx) || x > frmWidthEnd) return ;// out of screen on x
